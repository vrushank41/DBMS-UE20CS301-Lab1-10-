...
stations*/
select train_no,from_station_no,to_station_no,distance from route_info where to_station_no-from_station_no=1 order by train_no;
...
select train_no from route_info INNER JOIN compartment ON 
route_info.train_no=compartment.train_number where compartment.availability>10 and 
route_info.from_station_name='Bengaluru' and to_station_name='Chennai';
...
select firstname,lastname,dob,pnr from ticket left join train_user on ticket.passenger_id=train_user.user_id;

--6. Retrieve the first name, last name,of the Users who have not bought a ticket.
select distinct firstname,lastname from train_user left join ticket on train_user.user_id!=ticket.passenger_id;

/*RIGHT OUTER JOIN*/
--7. Retrieve the ticket PNR, Train number, travel date and along with all users first name and last name.
select pnr,train_no,travel_date,firstname,lastname from train_user right outer join ticket on train_user.user_id=ticket.passenger_id;
